...
- NPC 临时占位已经统一为人形比例轮廓，不再使用同尺寸色块。当前院落 NPC 尺寸：小明 `34x60`，二表哥 `38x74`，舅舅 `42x84`。外婆和舅妈虽然主要出现在室内，但也按同一系统记录为外婆 `42x78`、舅妈 `40x82`。
...
- 可调查物继续使用 `InteractableObject`，当前包括养鸡场、水井、圆石桌、晾衣绳、摩托、老屋门口、小卖部门口。
- 放在玩家行走带里的调查点必须设置 `blocks_player = false`，避免再次出现家具或道具挡路。
- 行动事件触发点建议 `interaction_priority = 5`，高于普通查看物(0)，低于 NPC(20)。

### 院落行动点分布（2026-05-30 首周事件包）

| 触发点 | 位置 | 事件 ID | 说明 |
|--------|------|---------|------|
| 养鸡场 | (185, 350) | `w1_act_feed_chickens` | 大屋左下方，摩托车右上方，不挡主路 |
| 水井 | (576, 382) | `w1_act_fetch_water` | 井边打水 |
| 石桌 | (392, 412) | `w1_act_stone_table_game` | 二表哥小游戏 |
| 晾衣绳 | (1002, 342) | `w1_act_collect_clothes` | 帮小明收衣服 |
| 摩托 | (220, 432) | `w1_act_check_scooter` | 舅舅摩托观察 |
| 老屋门口 | (750, 370) | `w1_act_old_house_eaves` | 老屋观察 |

NPC 站位：舅舅 (300, 430) 左侧院落，二表哥 (690, 410) 中段，小明 (1040, 420) 右侧。
