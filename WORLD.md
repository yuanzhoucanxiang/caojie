...
| 4 | 镜头 Y 偏移 | 天空在画面中占多少 | -150（联动缩放） | 构图重心、低机位仰视感 |
...
- 拉远（zoom=0.9）：偏移 = -150 × (1.25/0.9) ≈ -208 → 天空和楼体压迫感更强
- 默认（zoom=1.25）：偏移 = -150
- 拉近（zoom=2.5）：偏移 = -150 × (1.25/2.5) = -75 → 角色仍保留低机位感
...
**当前实现：**
- 基础偏移 = -150px（镜头在主角上方 150px）
- 缩放联动：`实际偏移 = -150 × (1.25 / zoom)`
- 拉远 0.9 → 偏移约 -208 → 天空、楼体和远景占比显著增大
- 默认 1.25 → 偏移 -150 → 角色偏下，形成孩子视角的轻微仰视
- 拉近 2.5 → 偏移约 -75 → 角色更清楚，但仍不完全居中
...
室内场景采用**横向舞台盒子**风格，参考《沉默意志》的暗色生活房间和《直到那时》的明亮儿童房。把一间屋子正面剖开给玩家看：墙面横向展开，人物站在中下部，家具像舞台布景一样分层摆放。

**设计原则：**
- 横向构图，信息沿 X 轴展开
- 正面剖面，不追求 3D 透视
- 后墙物件→中层家具→前景阴影，三层分明
- 局部光源（窗光、灶火、门洞暗部）制造视觉焦点
- 每个房间 8~15 个可命名物件，服务叙事而非装饰

**关键参数：** 地面线 `floor_y=340`，玩家走动 `y=350~410`，天花板 `ceiling_h=44~56`，侧墙厚 `wall=25~30`。室内门框高度控制在 `140~170px`，儿童角色约 `50~60px` 高，普通家具不要超过角色身高的 2 倍。

**三层大屋视觉定位：**

| 场景 | 画面性格 | 主要物件 |
|---|---|---|
| 一楼客厅/厨房 | 暗暖、烟火气 | 灶台、碗柜、圆桌、挂历、水缸 |
| 二楼走廊 | 窄、压缩 | 三个房门、奖状、歪画、狗垫 |
| 三楼主角房 | 蓝灰明亮 | 大窗光、床、书桌、行李箱、机器人 |

**镜头叙事：** 室外 zoom=1.25 → 一楼 1.8 → 三楼 2.0。镜头越贴近，房间越私密。天台拉远，形成”从家走向世界”的呼吸感。
...
- **统一比例尺**：童年主角正面游戏精灵按 `60px` 高作为儿童基准尺，脚底点就是角色节点原点。所有 NPC、家具和建筑都先和这个尺度比较，再决定自身尺寸。
- **人物高度**：儿童 NPC 约 `56~64px`；少年/二表哥约 `68~76px`；成年 NPC 约 `76~88px`；老人可略矮但体态更宽。不要让普通家具比儿童角色高太多，除非它是柜子、门或墙面物件。
- **当前 NPC 占位尺**：小明 `34x60`，二表哥 `38x74`，舅舅 `42x84`，外婆 `42x78`，舅妈 `40x82`。这些是临时人形轮廓尺寸，不是最终美术定稿；真实精灵接入时要保持脚底原点和这个身高关系。
- **室外建筑比例**：三层外婆大屋是家庭中心，当前院落默认镜头下约 `322×280px`；右侧黄土老屋是侧翼旧屋，约 `270×184px`。老屋必须明显低于大屋，不应接近大屋的视觉权重。
- **接地规则**：建筑不按图片画布中心摆放，而按真实墙脚/台阶底边对齐。院落建筑远景墙脚基线约 `y=348`，素材底边可向地面承接层咬入约 `5px`，并用接触阴影和 `GroundLip` 压住，避免贴纸浮空感。
- **门高参考**：普通室内门框 `140~170px`；室外主楼门洞可以略高，但仍要能和成年 NPC 尺寸对应，不要为了看清入口把门放大成建筑主角。
