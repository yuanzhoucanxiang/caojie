...
每个场景在搭建时，需要先确定以下参数来配合美术方向，让画面美观且各有特色：

**1. 建筑/地面落脚线（Ground Anchor）**

场景中所有建筑和落地物体的底部 Y 坐标，统一定在一条线上。这是"地面"的位置，也是深度范围的起点。

| 参数 | 说明 | 当前院子示例 |
|------|------|-------------|
| 落脚线 Y | 建筑底部对齐的 Y 坐标 | y=360 |

**2. 深度范围（Depth Range）**

主角前后走动的 Y 轴范围。上界对落脚线（建筑脚下），下界延伸到画面底部附近。范围越宽，玩家能走动的"深度感"越强。

| 参数 | 说明 | 当前院子示例 |
|------|------|-------------|
| DEPTH_MIN_Y | 深度上界 = 落脚线 | 360 |
| DEPTH_MAX_Y | 深度下界 ≈ 画面底边 | 520 |
| 范围 | 160px，约占画面 1/3 | |

**3. 镜头缩放范围（Zoom Range）**

室外场景支持鼠标滚轮缩放，让玩家自由调整视距。最远视距决定场景内容需要覆盖多大范围。

| 参数 | 说明 | 当前院子示例 |
|------|------|-------------|
| ZOOM_MIN | 最远（看到最多天空和全景） | 0.9 |
| ZOOM_MAX | 最近（特写、剧情聚焦） | 2.5 |
| ZOOM_DEFAULT | 默认游玩距离 | 1.25 |

**4. 镜头垂直偏移（Camera Y Offset）**

镜头中心相对于主角位置的 Y 轴偏移。偏移越大，天空在画面中占比越多。

| 参数 | 说明 | 当前院子示例 |
|------|------|-------------|
| 基础偏移 | 默认缩放时的偏移量 | -60 |
| 缩放联动 | 拉远时偏移增大（更多天空），拉近时减小（角色居中） | 按 zoom 比例 |

**5. 场景内容覆盖范围**

根据最远视距 + 最大深度 + 最大镜头偏移，计算场景内容（天空、地面、背景层）需要覆盖的总面积。确保最远视距下不会露出场景边缘。

**新场景搭建流程：**

1. 确定落脚线和深度范围 → 定 DEPTH_MIN_Y / DEPTH_MAX_Y
2. 确定缩放范围 → 定 ZOOM_MIN / ZOOM_MAX / ZOOM_DEFAULT
3. 确定镜头偏移 → 定 CAMERA_BASE_OFFSET
4. 计算内容覆盖面积 → 扩展天空/地面/背景层
5. 放置建筑和物体，底部对齐落脚线
6. 放置 NPC，Y 坐标在深度范围内
7. 测试：拉远 + 走到底，确认无场景边缘露出

**为什么每个场景要单独设定：** 不同的场景有不同的"画面性格"——院子开阔适合大范围深度、室内紧密适合窄范围、天台适合极端天空占比。这套参数是美术表达的骨架。
