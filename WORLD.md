...
| 特征 | 室内 | 室外 |
|------|------|------|
| 透视方式 | 平面剖面（舞台背景板） | 视差滚动+深度纵深 |
| 地板形状 | 平直矩形，全宽 | 不适用（自然地面） |
| 墙壁 | 窄垂直条（15px），像舞台侧幕 | 不适用 |
| 深度来源 | 光影渐变 + 微小缩放 | Y 轴移动 + 视差层 + 缩放 |
| 镜头偏移 | (0, 0)，居中 | (0, -100)，天空+开阔感 |
| zoom | 1.8~2.0（贴近角色） | 1.25（开阔视野） |
| 深度范围 | 50~70px（极窄） | 160px |
| 缩放范围 | 0.92~1.0（微妙） | 0.85~1.0 |

**房间盒子（Room Box）结构：**

每个室内场景由 5 个平面组成，全矩形、无梯形：

```
天花板：  顶部窄条（y=0~25），深灰褐，z 最深
后墙：    全宽矩形（25~340），米白，z 次深
左右墙：  窄垂直条（15px），暖灰褐，z 中间
地板：    全宽矩形（340~480），暖木色，z=0
```

**深度感不靠几何透视，靠三层叠加：**

1. **光影渐变**——天花板区域叠加深阴影（alpha 0.32），地面线附近叠加浅阴影（alpha 0.1），形成"房间深处→近处"的明暗过渡
2. **微小缩放**——玩家在 Y 移动范围（70px）内，scale 从 0.92（远处）到 1.0（近处），变化极微妙，不喧宾夺主
3. **z_index 深度排序**——所有物品的 z_index = 底部 Y 坐标，自然形成前后遮挡

**梯形透视（留给天台）：**

之前尝试的梯形地板+向内倾斜墙壁的透视方案，不适用于封闭室内——画面太窄、压迫感强。但适合天台这种"狭窄空间+向外瞭望"的场景：前宽后窄的地板 + 斜边墙 + 低消失点，强调"向外看"的开放感。

**各室内场景参数：**

| 场景 | 宽度 | zoom | 深度范围 | 特色 |
|------|------|------|---------|------|
| 1F 大屋 | 750px | 1.8 | 350~400 | 最宽的室内，生活核心区 |
| 2F 走廊 | 640px | 1.8 | — | 窄画面，走廊感 |
| 3F 主角房间 | 510px | 2.0 | 330~400 | 最近镜头，私密空间感 |
