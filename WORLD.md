...
每个场景在搭建之前，需要先确定一套构图参数。这些参数不是纯技术配置——它们直接决定了画面的"性格"：开阔还是亲密、仰望还是俯瞰、疏离还是沉浸。参数之间相互联动，修改一个会影响其他。
...
| # | 参数 | 决定什么 | 院子当前值 | 美术影响 |
|---|------|---------|-----------|---------|
| 1 | 场景地平线 Y | 地面区域起始+画面天/地分割 | 360 | 天空与地面的比例基准线 |
| 2 | 深度范围 | 主角能走动多远 | 360~520 (160px) | 画面纵深感、前景物布置空间 |
| 3 | 缩放范围 | 玩家能看多远/多近 | 0.9~2.5 | 全景感 vs 特写感的跨度 |
| 4 | 镜头 Y 偏移 | 天空在画面中占多少 | -100（联动缩放） | 构图重心、画面呼吸感 |
| 5 | 内容覆盖面积 | 场景素材画多大才够 | 天空-400~600、地面360~900 | 最远视距下不露边 |

---

##### 1. 场景地平线（Ground Anchor）

**定义：** 场景中"地面区域"的起始 Y 坐标。这是画面构图的分割线——以上是天空和远景，以下是地面和行走空间。主角深度范围的上界（DEPTH_MIN_Y）从这里开始。

**注意：** 这不是一条所有物体必须对齐的"线"。每个物体根据自己的场景设计位置有独立的落地 Y（深度锚点）。地平线只定义地面区域的起始位置和画面天/地分割比例。

**为什么重要：**
- 决定了画面中"天"和"地"的分割比例。线越靠上（Y 越小），地面越多；线越靠下，天空越开阔
- 主角深度范围的起点（DEPTH_MIN_Y = 地平线 Y）
- 建筑通常会放在地平线附近（表示它们坐落在"地面"上），但也可以放在更深的 Y 处（画面更前方）

**每个物体的独立深度锚点：**

物体不需要全部对齐到同一条 Y 线——它们可以布置在不同的深度位置。每个物体的深度锚点 = 父节点 Y + 最底部子件的 offset_bottom。这个值用于：
- z_index（视觉遮挡排序）
- ground_y（碰撞判定：|主角Y - ground_y| < 阈值 → 碰撞启用）

**院子取值：** 地平线 y=360，约在画面 2/3 处（75% 天空+远景，25% 地面）。当前大屋/老屋/龙眼树放在地平线附近（ground_y=360），表示它们在地面上。

**调整参考：**

| Y 值 | 天空占比 | 适合场景 |
|------|---------|---------|
| y=240 | 50%/50% | 闭合空间（村道、集市） |
| y=300 | 62%/38% | 半开放（院子内侧、天台） |
| y=360 | 75%/25% | 开阔户外（当前院子） |
| y=420 | 87%/13% | 极开阔（山顶、旷野） |

---

##### 2. 深度范围（Depth Range）

**定义：** 主角在 Y 轴上的走动范围（上下键前后走动）。上界 = 地平线（建筑脚后方），下界 ≈ 画面底部（最靠近镜头）。

**为什么重要：**
- 深度范围越宽，画面"纵深感"越强——玩家可以真实地走到建筑前面或后面
- 决定了前景物（y>390 的遮挡装饰）的布置空间
- 宽度影响镜头在不同 Y 位置时看到的天空/地面比例变化
- 碰撞系统按 |player.y - ground_y| < 阈值 判定——深度范围决定了玩家能否"走到碰撞发生的位置"

**调整参考：**

| 范围宽度 | 效果 | 适合场景 |
|---------|------|---------|
| 40~60px | 几乎固定在地面线上，只有左右移动 | 室内、走廊 |
| 80~120px | 有轻微前后走动感 | 村道、半开放空间 |
| 160~200px | 明显的前后深度 | 开阔院子、广场 |
| 200px+ | 强烈的纵深感 | 旷野、山顶全景 |

**深度范围与其他参数的联动：**
- 范围越宽 → 最远视距时间距越大 → 内容覆盖面积需要更宽
- 范围越宽 → 镜头 Y 偏移对构图的影响越明显（因为在不同深度位置看到的天/地比例变化大）

---

##### 3. 镜头缩放范围（Zoom Range）

**定义：** 室外场景支持鼠标滚轮实时缩放视距。ZOOM_MIN 是最远（画面拉远看到更多），ZOOM_MAX 是最近（画面拉近看到细节）。

**为什么重要：**
- 最远视距（ZOOM_MIN）是整个场景内容规划的基准——场景素材必须覆盖这个距离不被看穿
- 缩放范围决定了画面可表达的情绪跨度：日常散步用小 zoom、远眺全景用大 zoom
- 和剧情系统联动：剧情关键时刻可以锁 zoom 到特定值，做"推镜头"或"拉远"的演出
- 缩放联动镜头偏移：拉远时偏移增大（更多天空），拉近时偏移缩小（角色居中特写）

**镜头偏移联动公式：**

```
实际偏移 = CAMERA_BASE_OFFSET × (ZOOM_DEFAULT / 当前zoom)
```

- 拉远（zoom=0.9）：偏移 = -100 × (1.25/0.9) = -139 → 天空更多
- 默认（zoom=1.25）：偏移 = -100
- 拉近（zoom=2.5）：偏移 = -100 × (1.25/2.5) = -50 → 角色居中

**调整参考：**

| 场景类型 | ZOOM_MIN | ZOOM_MAX | ZOOM_DEFAULT | 理由 |
|---------|----------|----------|-------------|------|
| 开阔户外 | 0.9 | 2.5 | 1.25 | 全景+特写兼顾 |
| 半开放 | 1.0 | 2.0 | 1.3 | 不要太远以免露出边界 |
| 室内（平视舞台） | 1.8 | 2.0 | 1.8~2.0 | 贴近角色，不缩放 |
| 天台/山顶 | 0.7 | 2.0 | 1.0 | 极远强调开阔感 |

---

##### 4. 镜头垂直偏移（Camera Y Offset）

**定义：** 摄像机中心相对于主角位置的 Y 轴偏移量。负值 = 镜头上移（主角偏下、天空更多）。

**为什么重要：**
- 这是画面"构图重心"的控制器。偏移大 = 天空占比大、画面氛围开阔；偏移小 = 角色居中、聚焦叙事
- 与缩放联动：拉远时天空占比自然增大（符合"远眺"的视觉习惯），拉近时角色居中（符合"特写"的视觉习惯）
- 与落脚线配合：落脚线 Y + 镜头偏移 = "地平线在画面中的位置"
- 每种场景的"画面性格"很大程度上由这个参数塑造

**当前实现：**
- 基础偏移 = -100px（镜头在主角上方 100px）
- 缩放联动：`实际偏移 = -100 × (1.25 / zoom)`
- 拉远 0.9 → 偏移约 -139 → 天空占比显著增大
- 默认 1.25 → 偏移 -100 → 天空多于地面
- 拉近 2.5 → 偏移约 -50 → 角色接近画面中心

**调整参考：**

| 偏移 | 效果 | 适合场景 |
|------|------|---------|
| -150~-200 | 极大天空占比，仰视感 | 山顶、旷野 |
| -80~-120 | 天空多于地面，开阔但平衡 | 院子、村道 |
| -30~-60 | 角色偏画面中心 | 室内、集市 |
| 0~-20 | 角色几乎居中 | 对话特写、剧情演出 |

---

##### 5. 场景内容覆盖面积

**定义：** 场景内容（天空层、背景层、地面层、后处理覆盖层）需要覆盖的最小矩形区域。必须大于"最坏情况"下的相机可见范围。

**计算方法：**

```
相机最大可见范围 = {
    top:    主角最浅Y - (视口高度 / ZOOM_MIN / 2) + 最小镜头偏移
    bottom: 主角最深Y + (视口高度 / ZOOM_MIN / 2) + 最大镜头偏移
    left:   X边界 - (视口宽度 / ZOOM_MIN / 2)
    right:  X边界 + (视口宽度 / ZOOM_MIN / 2)
}
```
...
最坏情况（最大可见）：
- 主角最深 Y = 520（DEPTH_MAX_Y）
- 缩放 = 0.9（ZOOM_MIN）
- 镜头偏移 = -139（拉远联动）

相机中心 = 520 + (-139) = 381
半视高 = 480 / 0.9 / 2 = 267
可见 top = 381 - 267 = 114（天空层之上）
可见 bottom = 381 + 267 = 648（需要地面延伸到此处）

当前内容覆盖：天空 top=-400、地面 bottom=900 → 覆盖充分 ✓
```

**检查清单：**
1. 主角走到最深位置（y=520）→ 相机 bottom 不超过最底层素材
2. 主角走到最浅位置（y=360）→ 相机 top 不低于天空层上边界
3. 最远缩放（zoom=0.9）→ 上下左右都不露场景边
4. 后处理覆盖层大小 ≥ 内容覆盖面积
...
这 5 个参数不是孤立的——修改一个，其他要同步检查：

```
落脚线上移 → 深度范围和镜头偏移都要重新对齐
                    ↓
深度范围变化 → 内容覆盖面积重新计算
                    ↓
缩放范围变化 → 内容覆盖面积 + 镜头偏移联动公式都要重新算
                    ↓
镜头偏移变化 → 最远最深位置的可见范围重新验证
```
...
4. 算内容覆盖面积（确保素材够用）
5. 调镜头偏移（微调构图精细度）
6. 最后做全场景测试：拉远 + 走到底，检查四边

---

##### 新场景搭建流程

1. **确定画面性格** → 这个场景是开阔还是亲密？天空占比要大还是小？
2. **定地平线** → 从哪里分割天与地
3. **定深度范围** → 主角能走多大纵深
4. **定缩放范围** → 最远能看到多少、最近能看到多细
5. **定镜头偏移** → 画面重心偏上还是居中
6. **计算内容覆盖面积** → 天空/地面/背景层的素材需要多大
7. **放置建筑和物体** → 底部Y = 深度锚点(ground_y)，z_index = ground_y
8. **放置 NPC** → Y 坐标在深度范围内
9. **注册碰撞** → `_add_body(name, w, h, ground_y)`
10. **全场景测试** → 拉远 + 走到底四角，确认无边缘露出

**为什么每个场景要单独设定：** 不同场景的画面性格完全不同——院子开阔、室内亲密、天台仰望天空。这套五个参数互相联动的系统，就是美术表达的"技术骨架"。

---

##### 7. 室内场景构图——平视舞台

室内场景采用**平视剖面/舞台**风格，区别于室外的视差纵深体系。参考《直到那时（Until Then）》《Long Gone》的室内设计。

---

###### 7.1 设计哲学

室内是"家"的空间——亲切、温暖、有包裹感。跟室外的开阔正好相反：室外看到天、山、鱼塘，室内只看到房间和门窗。所以室内需要截然不同的画面语言。

**平视舞台 = 看舞台的正面剖面。** 不做立体透视，用平面矩形组成房间——像参观一栋建筑的切面模型。玩家感受到的是"在一个房间里"而不是"在一条走廊里"。这种风格在像素叙事游戏中非常常见，观众一眼就能理解空间关系。

**为什么不做 3D 透视？**

试过梯形地板+斜边墙的透视方案：在 854×480 的视口里，透视会让本来就窄的房间更窄。玩家的可走动区域被挤压到画面底部一条细带，视觉体验很差。而且透视要求美术素材为斜视角度，制作成本翻倍。最终决定把透视留给天台（那里窄空间向外看的透视感有叙事价值），室内保持平视。

**与室外的关系——镜头切换本身是叙事：**

从室外 zoom=1.25（开阔）切到室内 zoom=1.8~2.0（贴近），玩家感受到的是"走进屋里"的亲密感。3F 主角房间 zoom=2.0 是游戏中最贴近角色的镜头——暗示这是"自己的空间"。镜头切到室外 zoom=1.25 则像是"走出家门看到了世界"。天台切到 zoom=0.7 则是豁然开朗。
...
| 室内大厅 | 1.8 | 走进屋里，有包裹感 |
| 主角房间 | 2.0 | 贴近角色，私密空间 |
| 天台 | 0.7~1.0 | 走出束缚，看到天空 |

---

###### 7.2 房间盒子结构

每个室内场景由 5 个平面组成，全矩形。这 5 个面按照**绘制顺序**构建（从后到前），用 Polygon2D 节点实现：

```
第1步 天花板 (z=-100)  顶部长条，覆盖后墙上方区域
       ┌──────────────────────┐ y=0
第2步 天花板与后墙的交界线    │ y=25 (CEILING_H)
       │                      │
第3步 后墙 (z=-90)    全宽矩形，米白——这是最大的面，占据画面中上部
       │  ┌────────────┐      │ y=25
       │  │  窗户/门/海报 │     │ y=340 (FLOOR_Y)
       └──┴────────────┴──────┘
第4步 左右墙 (z=-80)   窄垂直条，15px 宽，从天花板到地面线
       ▓▓▓│              │▓▓▓   x=0~15, x=(W-15)~W
第5步 地板 (z=0)       全宽矩形，暖木色——玩家在这一层行走
       └──────────────────────┘ y=340~480
```
...
| 面 | 颜色 | 纹理 | z_index | 说明 |
|----|------|------|---------|------|
| 天花板 | `Color(0.35, 0.32, 0.26)` | NOISE, scale=100 | -100 | 最暗，暗示"抬头看" |
| 后墙 | `Color(0.82, 0.76, 0.62)` | NOISE, scale=100 | -90 | 最亮，舞台背景板 |
| 左右墙 | `Color(0.55, 0.48, 0.36)` | NOISE, scale=60 | -80 | 中间调，侧幕感 |
| 地板 | `Color(0.48, 0.38, 0.26)` | WOOD_H, scale=80 | 0 | 暖木色，水平方向 |

**关键参数：**

| 参数 | 1F | 2F | 3F | 说明 |
|------|----|----|-----|------|
| 屏幕宽度 (SCREEN_W) | 750 | 640 | 510 | 房间越小，镜头越近 |
| 地面线 (FLOOR_Y) | 340 | 340 | 340 | 所有室内统一 |
| 天花板高度 (CEILING_H) | 25 | 25 | 25 | 所有室内统一 |
| 墙壁厚度 (WALL_THICK) | 15 | 15 | 15 | 所有室内统一 |
| 后墙上下范围 | 25~340 | 25~340 | 25~340 | 所有室内统一 |

所有室内房间共享同一套天花板/地面线/墙壁参数——区别只有屏幕宽度和 zoom。镜头越近，屏幕越窄，房间感觉越小越私密。

---

###### 7.3 深度感——不靠几何透视，靠三层叠加

平视舞台没有梯形地板和斜边墙，深度感从哪里来？三层叠加制造空间暗示：

**第1层：光影渐变**

在后墙和天花板区域叠加两个 ColorRect 阴影层：

```
层1 深影：  位置 (0, 0), 尺寸 (SCREEN_W, CEILING_H + 50)
           颜色 (0.06, 0.04, 0.02, 0.32)
           → 房间上部/远处偏暗
           
层2 过渡影：  位置 (0, FLOOR_Y - 30), 尺寸 (SCREEN_W, 50)
           颜色 (0.06, 0.04, 0.02, 0.1)
           → 地面线附近有轻微明暗过渡
```

这样从画面顶部到地面线，亮度逐渐增加——远处暗、近处亮，这是最朴素的深度暗示。参考《直到那时》的室内光影：几乎每个场景都有自然的明暗层次，不需要严格的 3D 计算。

阴影层的 z_index=50，在所有房间面之上、后处理层之下（后处理 z=1000）。

**第2层：微小缩放**

玩家在 Y 移动范围内（330~400 或 350~400，取决于场景），scale 从 0.92 到 1.0 线性插值。

```
缩放公式：scale = lerp(0.92, 1.0, (position.y - DEPTH_MIN) / (DEPTH_MAX - DEPTH_MIN))
```

- 在远处（Y 较小）→ scale 0.92，角色微微缩小，感觉"在房间里面"
- 在近处（Y 较大）→ scale 1.0，角色恢复原大，感觉"在画面前面"

变化极其微妙——8% 的缩放差，肉眼几乎察觉不到单次移动时的变化，但累积下来会有空间感。

**第3层：z_index 深度排序**

所有物品的 z_index = 底部 Y 坐标。地板和物品按 z_index 排序：底部 Y 更大的物体渲染在上面（更靠近玩家）。这跟室外的遮挡规则一致——统一用"最底部子件的 Y 坐标"作为深度锚点。

玩家的 z_index = int(position.y)（动态更新）。

**三层的关系：**

| 层 | 控制什么 | 从哪里来 |
|----|---------|---------|
| 光影渐变 | 明暗分层 | 两个 ColorRect 阴影叠加 |
| 微小缩放 | 物体大小暗示距离 | player.gd `_update_depth_scale()` |
| z_index 排序 | 物体遮挡关系 | 各节点的 z_index 属性 |

三层独立工作，互相配合。光影决定明暗，缩放决定大小，z_index 决定遮挡。单一哪一层都不能独立制造深度感，三层叠加才有空间暗示。
...
| 参数 | 值 | 说明 |
|------|-----|------|
| zoom | 1.8~2.0 | 贴近角色，不缩放 |
| camera.offset | (0, 0) | 居中——室内不需要天空+开阔感 |
| camera.limit_* | 各室内不同 | 防止镜头看到场景外 |
| depth_speed | 200 | 室内外统一 |
| 滚轮缩放 | 禁用 | 室内 zoom 固定 |

室内不支持滚轮缩放（`_can_zoom()` 返回 false for `house_*` 场景）。Zoom 值写死在 SceneManager.CAMERA_CONFIGS 里，切到室内时直接设置。

**移动范围：**

| 场景 | X 范围 | Y 范围 | Y 宽度 |
|------|--------|--------|--------|
| 1F | 20~730 | 350~400 | 50px |
| 2F | 16~624 | — | — |
| 3F | 20~490 | 330~400 | 70px |

Y 范围很窄（50~70px），这是刻意的——室内场景玩家主要是左右走动，上下走动只是"调整深度"。这个设计类似《爱氏物语》的室内：角色在地面线上下小幅移动，大部分时间沿水平方向探索。

**出生点规则：**

每个室内场景定义 SPAWN_POINTS 常量：
```gdscript
const SPAWN_POINTS := {
    "from_<来源>": Vector2(x, y),
}
```
- 出生点 Y 必须在深度范围内（330~400）
- 出生点 X 根据入口位置决定（楼梯在左，门在右，或相反）
- 门的位置在地面线 Y=340 上

**相机边界：**

SceneManager 的 CAMERA_CONFIGS 中为每个室内配置 limits：
...
- limits 防止镜头看到场景外的紫色背景
- player_bounds 是玩家的移动边界，通过 `player.set_movement_bounds()` 设置
- 室内的 player_bounds Y 范围很窄（50~70px），X 范围根据屏幕宽度决定（留 20~25px 侧墙空间）

---

###### 7.5 家具与可互动物品

室内场景的核心是**可互动的物品**——玩家在房间里不仅走动，还能跟物品互动。

**InteractableObject 系统：**

这是一个复用现有对话系统的轻量方案：

```
Player 按 E → InteractableObject 检测 _in_range
  → 构造 event_data: {"text": 描述文字, "choices": [{"text": "好的"}]}
  → dialogue_request.emit() → DialogueManager 显示气泡
```

InteractableObject 继承 StaticBody2D（天然有碰撞体），内部包含 Area2D 用于检测玩家接近。

**物品视觉设计：**

室内物品用 Polygon2D 矩形组成，配合 procedural_texture.gdshader 纹理。一个物品可以由多个子件组成（如桌子 = 桌面 + 桌腿 + 横梁），每个子件的 z_index = 底部 Y 坐标，保持深度排序。

颜色方案要温暖、不突兀——所有颜色必须符合"不出现纯白纯黑"的美术规则。
...
**关键布局原则：**
- 床靠左墙，书桌靠右墙——中间留出宽敞走道（约 250px）
- 窗户在后墙中央偏上——玩家抬头能看到窗外
- 地毯标记走道区域
- 椅子紧贴书桌，但不要有碰撞——玩家可以走到书桌前
- 小物品（笔记本、铅笔、台灯）堆在书桌上面，增加生活气息

---

###### 7.6 室内场景间的关系

3F → 2F → 1F 是一栋楼的上下层关系。它们之间通过楼梯 Trigger 连接，Trigger 设置 `spawn_id` 指向目标场景的出生点。

| 路径 | Trigger | spawn_id | 出生位置 |
|------|---------|----------|---------|
| 1F → 2F | StairsUp (右边) | "from_1f_up" | 2F 左侧 |
| 2F → 3F | StairsUp (右边) | "from_2f_up" | 3F 左侧 |
| 3F → 2F | StairsDown (左边) | "from_2f_down" | 2F 右侧 |
| 2F → 1F | StairsDown (左边) | "from_2f_down" | 1F 右侧 |
| 1F → 庭院 | ExitDoor (左边) | "from_house" | 庭院中右边 |
| 庭院 → 1F | HouseDoor (右边) | "from_outside" | 1F 左边 |

楼梯在房间右侧（玩家从下层走上来时出现在下层的右侧），出生点在上层的左侧（楼梯口的另一侧）。玩家"上楼梯"等于"走到上层的另一端"。

**视觉通感：**

楼梯触发器的位置应靠近墙壁上的某个视觉标识（如楼梯图标色块），让玩家知道"这里可以上楼/下楼"。

---

###### 7.7 梯形透视（留给天台）

梯形地板（前宽后窄）+ 斜边墙 + 低消失点的透视方案不适用于封闭室内——在 854×480 的视口里太窄。但适合天台：

- **天台特性**：狭窄空间 + 向外瞭望
- **梯形地板**：前宽后窄，暗示空间向外延伸
- **斜边墙**：左侧墙和右侧墙向消失点汇聚
- **低消失点**：玩家视线向下，看向地平线和远处
- **camera.zoom 可开启**：天台属于室外，允许滚轮缩放，配合透视效果更强烈

天台的实现细节待定——目前 3F 的天台触发器（Rooftop）使用 `target_scene_path = "res://scenes/main.tscn"`，切换到庭院后处理。后续可以做一个独立的天台场景。
...
1. **确定房间宽度和 zoom**——屏幕越窄，zoom 越高，房间感越小越私密
2. **创建房间盒子的 5 个面**——天花板→后墙→左墙→右墙→地板，用 Polygon2D + ShaderMaterial
3. **隐藏旧的 tscn 平面节点**（如果有）——`_hide_old_nodes()` 把旧的 ColorRect 设为 `modulate.a = 0`
4. **设置光影渐变**——两个 ColorRect 阴影叠加在后墙和地面线区域
5. **配置 SceneManager CAMERA_CONFIGS**——zoom、offset、limits、player_bounds
6. **设置墙壁碰撞体**——左右墙 StaticBody2D
7. **放置家具**——Polygon2D 组成的可碰撞物品（如有需要）
8. **添加可互动物品**——InteractableObject 实例，设置描述文字和提示文字
9. **设置出生点**——SPAWN_POINTS 常量，匹配进入方向
10. **设置 transition trigger**——楼梯或门的 TransitionTrigger，target_area_id + spawn_id

**为什么每个房间要单独设定宽度：**

| 宽度 | zoom | 心理效果 | 适合 |
|------|------|---------|------|
| 750px | 1.8 | 最宽的室内，生活区 | 大屋一楼（客厅/厨房/生活区） |
| 640px | 1.8 | 窄，走廊感 | 大屋二楼（走廊/卧室） |
| 510px | 2.0 | 最窄，最贴脸 | 主角房间（私密空间） |

宽度决定"这个房间有多少可探索内容"。3F 主角房间只有 510px——画面紧，但 zoom 最高，细节最清晰，玩家感受到"这是我自己的小空间"。

---

#### 场景事物遮挡与碰撞设计

场景中的每个事物通过**两层独立系统**实现 2.5D 空间关系：视觉层处理遮挡、物理层处理碰撞。两层各司其职，互相独立。

**视觉遮挡层（z_index 深度排序）**

每个物体的 z_index = 它的深度锚点（ground_y，即最底部落地 Y）。主角的 z_index = int(position.y)（动态）。Y 值越大，z_index 越高，渲染在越前面。

**物理碰撞层（CollisionBody）**

碰撞按深度位置动态判定：`|主角Y - 物体ground_y| < 25px` → 碰撞启用；超出 → 碰撞关闭。物体和主角只有在"同一深度面"时才会碰撞。

前景遮挡物（ground_y 超出主角深度范围）处于不同深度空间，主角经过只视觉遮挡，不会碰撞。

碰撞体宽度匹配物体实际尺寸，厚度 20px。

**深度判定逻辑：**

```
主角Y < ground_y - 25  → 后面：物体遮挡主角，不碰撞
主角Y ≈ ground_y ±25  → 同面：碰撞启用
主角Y > ground_y + 25  → 前面：主角在物体前，不碰撞
```

**多部分组合物体的统一深度规则：**

所有物体——无论是单色块还是树/建筑等多部分组合——深度位置统一按**最底部子件的落地点 Y** 来计算。视觉遮挡（z_index）和物理碰撞（ground_y）共用同一个值。

| 规则 | 说明 |
|------|------|
| 深度锚点 | 物体最底部子件的 offset_bottom + 父节点 Y |
| z_index | = 深度锚点 Y（整棵树冠+干统一值） |
| 碰撞 ground_y | = 深度锚点 Y（同深度时才碰撞） |

例：树冠画在 y=320~352、树干底在 y=400。整棵树的 z_index=400、ground_y=400。

**碰撞体分类：**

| 事物类型 | 碰撞 | 说明 |
|---------|:--:|------|
| 建筑（大屋/老屋） | ✓ | 与主角同深度面，脚下薄条 |
| 树木（龙眼树树干底部） | ✓ | 同深度面 |
| 设施（井/晾衣绳/鸡棚） | ✓ | 同深度面 |
| 前景木桩/暗边 | ✗ | 不同深度面，纯视觉遮挡 |
| 前景大树/栅栏/草丛 | ✗ | 不同深度面，纯视觉遮挡 |

**设计原则：**
- 视觉遮挡由 z_index 和 Y 坐标自动控制，所有事物参与
- 物理碰撞只看该事物是否与主角处于同一深度空间（落地在 y≈360 地面线）
- 主角从前景物"后面"走过 → 视觉被遮但不碰撞，这是正确的 2.5D 行为
- 剧情演出另设场景，不受日常遮挡规则约束

#### 场景切换

场景切换使用**推拉幕布过渡**——场景主色调的纯色块从左滑入覆盖画面，切场景后往右滑出揭示新画面。

| 阶段 | 画面效果 |
|------|---------|
| 滑入 | 暗沉版场景主色调从左边滑入(0.3s) |
| 切换 | 幕布完全覆盖时切换场景+调整镜头 |
| 滑出 | 幕布往右滑出(0.3s)，新画面显现 |
...
**参考**：Celeste / Hollow Knight 的推拉幕布过渡。
