...
规则：场景中所有"落地"的事物都应该有物理碰撞——无论它是建筑、树木、还是前景遮挡物。碰撞体统一使用脚下的薄矩形条（6~8px 高），宽度与实际物体匹配。
...
| 事物类型 | 碰撞体形状 | 示例 |
|---------|-----------|------|
| 建筑（大屋/老屋） | 薄条 8px × 实际宽度 | 大屋 190×8 |
| 树木（树干底部） | 薄条 6px × 树干宽度 | 龙眼树 12×6 |
| 井/晾衣绳/鸡棚 | 薄条 6px × 实际宽度 | 水井 22×6 |
| 前景木桩/暗边/栅栏 | 薄条 6px × 实际宽度 | 木桩 13×6 |
| 前景大树 | 薄条 6px × 树干宽度 | 前景树 20×6 |

**设计原则：**
- 视觉遮挡由场景设计师通过 z_index 和 Y 坐标控制
- 物理碰撞只看脚下落地点——不堵头、不卡视觉
- NPC 自由活动时不会"卡"在遮挡物后面——碰撞薄条在脚下，绕一下就能走开
- 剧情演出另设场景，不受遮挡规则约束
