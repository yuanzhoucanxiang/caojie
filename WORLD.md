...
**定义：** 主角在 Y 轴上的走动范围（上下键前后走动）。上界 = 落脚线（建筑脚后方），下界 ≈ 画面底部（最靠近镜头）。
...
- 拉远 0.9 → 偏移约 -139 → 画面 80% 天空
- 默认 1.25 → 偏移 -100 → 画面 70% 天空
- 拉近 2.5 → 偏移约 -50 → 画面 55% 天空
...
7. **放置建筑和物体** → 底部对齐落脚线，z_index = 落地点 Y
8. **放置 NPC** → Y 坐标在深度范围内
...
所有场景事物（建筑、角色、NPC、前景物）按 Y 坐标自动排序：`z_index = int(position.y)`。Y 值越大（越靠近镜头），z_index 越高，渲染在越前面。这是统一的深度排序，无需手动设置。

**物理碰撞层（CollisionBody）**

规则：只有在深度面上与主角处于同一空间的事物才需要物理碰撞——即落地在 y≈360 地面线上的建筑、树木、设施。前景遮挡物（y>390）处于不同的深度空间，主角从它们"后面"走过，只需视觉遮挡，不应碰撞。

碰撞体统一使用脚下的薄矩形条（6~8px 高），宽度与实际物体匹配。
...
|---------|------|:--:|
| y<360 | 天空/远景（视觉背景） | ✗ |
| y≈360（地面线） | 建筑/树/井/设施（与主角同空间） | ✓ |
| y=360~520 | 主角/NPC 走动范围 | — |
| y>390 | 前景遮挡物（木桩/栅栏/草丛/大树） | ✗ 仅视觉遮挡 |
