...
| # | 参数 | 决定什么 | 院子当前值 | 美术影响 |
|---|------|---------|-----------|---------|
| 1 | 地面落脚线 Y | 所有建筑/物体的底部对齐位置 | 360 | 天空与地面的比例分割线 |
...
##### 1. 地面落脚线（Ground Anchor）

**定义：** 所有建筑、树木、设施等落地物体的底部 Y 坐标，统一对齐到同一条线上。

**为什么重要：**
- 这是整个场景深度空间的"原点"——所有物体的 z_index 和碰撞 ground_y 都以此为基准
- 落脚线的位置决定了画面中"天"和"地"的分割比例。线越靠上（Y 越小），天空越少、地面越多；线越靠下，天空越开阔
- 这条线也是主角深度范围的起点（DEPTH_MIN_Y = 落脚线 Y）
- 像树这样的多部分组合物体，整棵树的深度锚点也取树干底部的落脚线位置

**院子取值：** y=360，约在画面 2/3 处（360/480=75%，即画面 75% 是天空+远景，25% 是地面）。这是开阔户外场景的典型"地平线"位置。
