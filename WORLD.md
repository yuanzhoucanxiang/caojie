...
**平视舞台 = 看建筑的正面切面。** 不做 3D 透视，用平面矩形组成房间——天花板、后墙、左右墙、地板，五个面构成一个舞台布景。玩家像在看一栋建筑的剖面模型。这种风格在像素叙事游戏里非常普遍：观众一眼就能理解空间关系，美术素材是正面的、平面的，制作门槛低。

**为什么不用 3D 透视：**

试过梯形地板+斜边墙的透视方案——854×480 视口下，透视让本来就窄的房间更窄，玩家可走动区域被挤压到画面底部细带。而且透视要求美术素材为斜视角度，制作成本翻倍。室内追求的是"包裹感"而非"空间感"，平视的正面剖面天然就传递包裹感。梯形透视留给天台——狭窄空间向外看的透视感有明确的叙事价值。

**镜头切换本身是叙事：**

室外 zoom=1.25 → 室内 zoom=1.8~2.0，玩家感受到"走进屋里"的亲密。镜头越贴近，房间感觉越小越私密。3F 主角房间 zoom=2.0 是游戏中最贴近角色的镜头——暗示"这是我的空间"。切到室外 zoom=1.25 则是"走出家门看到了世界"。切到天台 zoom=0.7 是豁然开朗。

**深度感不靠几何透视，靠三层叠加：**

没有梯形地板和斜边墙，深度感从哪来？

1. **光影渐变**——天花板区域叠深阴影，地面线附近叠浅阴影，形成"房间深处偏暗、近处偏亮"的明暗过渡。参考《直到那时》的室内：几乎所有室内场景都有自然的明暗层次，不需要精确的 3D 计算。
2. **微小缩放**——玩家上下移动时，角色 scale 在 0.92~1.0 之间微妙变化。8% 的缩放差肉眼几乎察觉不到，但累积下来有空间暗示。不喧宾夺主。
3. **z_index 排序**——所有物品的 z_index = 底部 Y 坐标，自然形成前后遮挡。跟室外的深度排序规则一致。

三层独立工作，互相配合。单一哪一层都不能独立制造深度感。

**每个室内场景单独设计：**

当前 3F 主角房间的参数（地面线 y=340、天花板 25px、墙壁 15px）是原型验证，不代表所有室内都照搬。不同房间有不同比例——比如走廊更扁、灶台间更矮、杂物间更暗。zoom 也是逐场景调整的，3F=2.0（最近最私密），1F=1.8（稍远一点的大厅），如果以后有暗室或阁楼，zoom 可以到 2.2 以上。宽度同理，不是固定的几个档位，而是每个场景根据它想传达的感觉单独定。

只有一条不变：**地面线（y=340）**。这条线是室内外所有房间的基准——玩家的落脚点永远在地面线上，门也踩在地面上。不管房间多宽多高，人物的行走平面是统一的。

**室内物品的互动性：**

室内场景的核心是**可互动的物品**。InteractableObject 系统复用现有对话气泡——玩家靠近按 E，弹出描述气泡。不需要新的 UI。物品用 Polygon2D 矩形拼接，颜色温暖不突兀，符合"不出现纯白纯黑"的美术规则。

布局上，房间要留出**宽敞走道**——床靠左墙、书桌靠右墙，中间留 ~250px 的走动空间。小物品（笔记本、书本、地毯）增加生活气息。窗户在后墙，玩家抬头能看到外面。

**室内场景之间的关系：**

1F→2F→3F 是一栋楼的上下层。楼梯 Trigger 设置 `spawn_id` 指向目标场景的出生点，玩家"上楼"等于"走到上层的另一端"。门在左边、楼梯在右边（或反过来），让玩家在楼层间有方向感。从大屋出门回到庭院，zoom 从 1.8 切回 1.25——镜头拉远，视野打开。

**梯形透视留给天台：**

天台是"狭窄空间+向外瞭望"——梯形前宽后窄的地板 + 斜边墙 + 低消失点，强调"向外看"的开放感。天台属于室外，允许滚轮缩放，配合透视效果更强烈。天台的实现细节待定。

**搭建要点（搭新室内场景时的参考）：**

搭建一个室内场景，核心就是三件事：**搭房间、设镜头、放东西**。

- **搭房间**：5 个 Polygon2D 面（天花板→后墙→左右墙→地板），从后到前 z_index 递增。旧的 tscn 平面节点隐藏掉（`modulate.a = 0`）。用 `_add_depth_lighting()` 叠两个阴影层。
- **设镜头**：在 `SceneManager.CAMERA_CONFIGS` 里加一条该场景的配置——zoom、offset(0,0)、limits（防看到场景外）、player_bounds（X 范围留侧墙空间、Y 范围在地面线上下 50~70px）。
- **放东西**：墙壁碰撞用 StaticBody2D，家具用 Polygon2D 矩阵拼接（可碰撞可不碰），可互动物品用 InteractableObject（`setup()` 设名字/描述/尺寸，自动接入对话气泡）。
- **门和楼梯**：用 TransitionTrigger，`target_area_id` 指向目标场景，`spawn_id` 指向目标场景的出生点。门/楼梯踩在地面线（y=340）上。

参考 `house_floor3.gd`（3F 主角房间）的代码结构——它是最完整的室内场景示例。新场景复制这个脚本改参数即可。
